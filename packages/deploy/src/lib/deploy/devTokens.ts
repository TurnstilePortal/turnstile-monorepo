import type { Hex } from 'viem';
import type { AztecAddress, Wallet as AztecWallet } from '@aztec/aztec.js';

import { AztecToken } from '@turnstile-portal/turnstile.js';

import type { L1ComboWallet } from '@turnstile-portal/turnstile-dev';
import { InsecureMintableToken } from '@turnstile-portal/turnstile-dev';

const ADDITIONAL_L1_ADDRESSES_TO_FUND: `0x${string}`[] = [
  '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266',
  '0x70997970C51812dc3A010C7d01b50e0d17dc79C8',
  '0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC',
  '0x90F79bf6EB2c4f870365E785982E1f101E93b906',
  '0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65',
  '0x9965507D1a55bcC2695C58ba16FB37d819B0A4dc',
  '0x976EA74026E726554dB657fA54763abd0C3a0aa9',
  '0x14dC79964da2C08b23698B3D3cc7Ca32193d9955',
  '0x23618e81E3f5cdF7f54C3d65f7FBc0aBf5B21E8f',
  '0xa0Ee7A142d267C1f36714E4a8F75612F20a79720',
];

export async function deployL1DevToken(
  wallet: L1ComboWallet,
  name: string,
  symbol: string,
  decimals: number,
) {
  const token = new InsecureMintableToken(wallet.wallet, wallet.public);
  const tokenAddr = await token.deployInsecureMintableToken(
    name,
    symbol,
    decimals,
  );
  console.log(`InsecureMintableToken deployed at ${tokenAddr}`);
  fundL1DevToken(wallet, tokenAddr, BigInt(1000e18));
  return tokenAddr;
}

export async function fundL1DevToken(
  wallet: L1ComboWallet,
  tokenAddr: Hex,
  amount: bigint,
) {
  const token = new InsecureMintableToken(wallet.wallet, wallet.public);

  await token.mint(tokenAddr, wallet.wallet.account.address, amount);
  console.log(
    `InsecureMintableToken(${tokenAddr}) minted ${amount} for ${wallet.wallet.account.address}`,
  );
  for (const a of ADDITIONAL_L1_ADDRESSES_TO_FUND) {
    await token.mint(tokenAddr, a, amount);
    console.log(
      `InsecureMintableToken(${tokenAddr}) minted ${amount} for ${a}`,
    );
  }
}

export async function deployL2DevToken(
  wallet: AztecWallet,
  aztecPortal: AztecAddress,
  name: string,
  symbol: string,
  decimals: number,
): Promise<Hex> {
  const token = await AztecToken.deploy(
    wallet,
    aztecPortal,
    name,
    symbol,
    decimals,
  );
  console.log(`AztecToken deployed at ${token.address()}`);
  return token.address();
}
