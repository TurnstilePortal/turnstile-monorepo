...
import { type DeploymentData, getChain, getClients, writeDeploymentData } from '@turnstile-portal/turnstile-dev';
...
export function registerDeployCommand(program: Command) {
  return program
    ...
    .requiredOption('-c, --config-dir <directory>', 'Config directory containing config.json, keys.json, etc.')
    .option('-o, --overwrite', 'Overwrite existing deployments and keys', false)
    .action(async (options) => {
      try {
        console.log(`Starting deployment using config directory: ${options.configDir}`);
        ...
        if (!existsSync(configPaths.configFile)) {
          console.log(`Config file not found, creating default: ${configPaths.configFile}`);
        ...
        if (deploymentExists) {
          console.log(`Found existing deployment data at ${configPaths.deploymentFile}`);
        ...
        if (config.setup) {
          ...
          if (!setup) {
            console.warn(`Warning: Setup "${config.setup}" specified but not found in registry`);
            console.log('Available setup implementations:', Object.keys(setupRegistry).join(', ') || 'None');
          } else {
 ...
 */
async function runDeployment(config: DeployConfig, paths: { keysFile: string; deploymentFile: string }): Promise<void> {
  ...
  if (deploymentExists && !config.deployment.overwrite) {
    throw new Error(`Deployment file already exists at ${paths.deploymentFile}. Use --overwrite to redeploy.`);
  }
  ...
  try {
    ...
    const deploymentResult = await deployTurnstileContracts(l1Client, l2Client, { registryAddress });
    ...
    deploymentData = {
      ...
      serializedAztecPortalInstance: deploymentResult.serializedAztecPortalInstance,
      aztecShieldGateway: deploymentResult.aztecShieldGateway,
      serializedShieldGatewayInstance: deploymentResult.serializedShieldGatewayInstance,
    };

    // Save deployment data
    console.log('Saving deployment data...');
    await writeDeploymentData(paths.deploymentFile, deploymentData as DeploymentData);
    ...
    if (Object.keys(deploymentData).length > 0) {
      console.log('Deployment failed. Saving partial deployment data for debugging...');
      await writeDeploymentData(paths.deploymentFile, deploymentData as DeploymentData);
    }
    throw error;
  }
  ...
  if (hasTokens && deploymentData.aztecPortal) {
    ...
    for (const [symbol, tokenConfig] of Object.entries(config.deployment.tokens)) {
      ...
      try {
        if (!deploymentData.l1AllowList || !deploymentData.aztecPortal || !deploymentData.l1Portal) {
          throw new Error('Missing required deployment data for token deployment');
        }

        // Deploy single token
        const result = await deployToken(l1Client, l2Client, deploymentData.aztecPortal, tokenConfig);

        // Propose the token
        await proposeL1DevToken(l1Client, result.l1Address, deploymentData.l1AllowList);

        // Accept the token
        await acceptL1DevToken(l1Client, result.l1Address, deploymentData.l1AllowList);
        ...
        const { messageHash, messageIndex, l2BlockNumber } = await registerL1DevToken(
          l1Client,
          result.l1Address,
