...
import { createError, ErrorCode, type ErrorContext, TurnstileError } from './errors.js';
...
describe('TurnstileError', () => {
  ...
  it('should return the correct category based on error code', () => {
    const l1Error = new TurnstileError(ErrorCode.L1_TOKEN_OPERATION, 'L1 error');
    const l2Error = new TurnstileError(ErrorCode.L2_TOKEN_OPERATION, 'L2 error');
    const bridgeError = new TurnstileError(ErrorCode.BRIDGE_DEPOSIT, 'Bridge error');
    const configError = new TurnstileError(ErrorCode.CONFIG_INVALID_PARAMETER, 'Config error');
    const validationError = new TurnstileError(ErrorCode.VALIDATION_AMOUNT, 'Validation error');
  ...
  it('should format toString with context and cause', () => {
    ...
    expect(errorString).toContain(`(${ErrorCode.VALIDATION_ADDRESS} - Validation)`);
  ...
  it('should format toString without context or cause', () => {
    const error = new TurnstileError(ErrorCode.L1_TIMEOUT, 'Operation timed out');
    const errorString = error.toString();
...
describe('createError function', () => {
  it('should create errors with any error code without validation', () => {
    // Test L1 error codes
    const l1Error = createError(ErrorCode.L1_TOKEN_OPERATION, 'Token operation failed', { operation: 'transfer' });
    ...
    // Test L2 error codes
    const l2Error = createError(ErrorCode.L2_SHIELD_OPERATION, 'Shield operation failed', { amount: '100' });

    expect(l2Error.code).toBe(ErrorCode.L2_SHIELD_OPERATION);
    expect(l2Error.category).toBe('L2');

    // Test Bridge error codes
    const bridgeError = createError(ErrorCode.BRIDGE_DEPOSIT, 'Bridge deposit failed', { txHash: '0xabc' });

    expect(bridgeError.code).toBe(ErrorCode.BRIDGE_DEPOSIT);
    expect(bridgeError.category).toBe('Bridge');

    // Test Config error codes
    const configError = createError(ErrorCode.CONFIG_MISSING_PARAMETER, 'Missing required parameter', {
      param: 'apiKey',
    });
    ...
    const validationError = createError(ErrorCode.VALIDATION_RANGE, 'Value out of range', {
      value: 101,
      min: 0,
      max: 100,
    });
  ...
  it('should allow cross-category error code usage (no range restrictions)', () => {
    ...
    const l1BridgeError = createError(ErrorCode.BRIDGE_WITHDRAW, 'L1 operation encountered bridge withdrawal error', {
    ...
    const l2ValidationError = createError(ErrorCode.VALIDATION_ADDRESS, 'L2 operation encountered validation error', {
  ...
  it('should support complex nested context objects', () => {
    ...
    const error = createError(ErrorCode.L1_TOKEN_OPERATION, 'Complex error with nested context', context);
