...
async function doTransfer(
  ...
  console.log(
    `PRIVATELY Transferring using PUBLIC CHANNELS ${amount} ${symbol} to ${recipient}...`,
  );
  ...
  const { authWitness, nonce } =
    await token.createAuthWitnessForPrivateTransfer(recipient, amount);

  const tx = await token.transferPrivateWithPublicChannel(
    recipient,
    amount,
    nonce,
    authWitness,
...
export function registerAztecTransferPrivateChannel(program: Command) {
  return program
    .command('aztec-transfer-private-channel')
    .description('Transfer Aztec tokens privately using public channels')
