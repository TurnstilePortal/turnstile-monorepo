...
async function doUnshield(wallet: Wallet, token: AztecToken, amount: bigint) {
  const symbol = await token.symbol();
  console.log(`Unshielding ${amount} ${symbol}...`);

  const balance = await token.balanceOfPrivate(wallet.getAddress());
...
export function registerUnshieldTokens(program: Command) {
  return program
    ...
    .action(async (options) => {
      ...
      const tx = await doUnshield(wallet, token, amount);
