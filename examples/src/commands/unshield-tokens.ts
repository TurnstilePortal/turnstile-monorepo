...
async function doUnshield(l2Client: L2Client, token: L2Token, amount: bigint) {
  ...
  const balance = await token.balanceOfPrivate(l2Client.getAddress()).simulate({ from: l2Client.getAddress() });
  console.log(`Current private balance: ${balance}`);
  if (balance < amount) {
    throw new Error('Insufficient balance.');
  }

  const tx = token.unshield(amount).send({ from: l2Client.getAddress() });
...
export function registerUnshieldTokens(program: Command) {
  return program
    ...
    .action(async (options, command) => {
      ...
      // Create token & ensure it is registered in the PXE
