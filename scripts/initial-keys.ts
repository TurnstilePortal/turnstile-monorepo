...
export const INITIAL_TEST_ENCRYPTION_KEYS = INITIAL_TEST_SECRET_KEYS.map(
  (secretKey) => deriveMasterIncomingViewingSecretKey(secretKey),
);
...
for (let i = 0; i < INITIAL_TEST_SECRET_KEYS.length; i++) {
  console.log(
    `Secret key: ${INITIAL_TEST_SECRET_KEYS[i].toString()}, Signing key: ${INITIAL_TEST_SIGNING_KEYS[i].toString()}, Salt: ${INITIAL_TEST_ACCOUNT_SALTS[i].toString()}`,
  );
}
